...
shinyData <- data.frame(read.csv("https://raw.githubusercontent.com/GUODONGTAN/cassandraNFT/main/shinyData.csv"))
...
transactionData <- data.frame(read.csv("https://raw.githubusercontent.com/GUODONGTAN/cassandraNFT/main/transaction.csv"))
...
shinyServer(function(input, output) {
  ...
  output$mean = renderText(paste(mean(as.numeric(transactionData$amount[transactionData$name == input$selector]), na.rm = TRUE)))
  output$std = renderText(paste(sd(as.numeric(transactionData$amount[transactionData$name == input$selector]))))
  output$plot1 <- renderPlot({plot(transactionData$amount[transactionData$name ==input$selector],
                                   type = "l",
                                   lwd = 3,
                                   col="navy",
                                   xlab = "Time(N days ago)",
                                   ylab = "Price(ETH)",
                                   main = input$selector)},
                             res = 96)
